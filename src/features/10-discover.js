...
const followingStories = [
  { icon: "S", name: "Songbyrd", type: "Venue", intro: "What's coming up at Songbyrd over the next few days.", eventIds: [1, 11] },
  { icon: "D", name: "@dcafterdark", type: "Curator", intro: "A few things @dcafterdark thinks are worth leaving the house for this week.", eventIds: [4, 6, 7] },
  { icon: "R", name: "DC Run Club", type: "Public group", intro: "Upcoming ways to run, meet people, and hang out with DC Run Club.", eventIds: [5, 8, 2] },
  { icon: "M", name: "Smithsonian", type: "Venue", intro: "A Smithsonian event to catch over the next couple of days.", eventIds: [9] },
  { icon: "E", name: "@eater_dc", type: "Curator", intro: "Food and drink suggestions from @eater_dc for the next few days.", eventIds: [2, 4, 8] },
  { icon: "A", name: "Atlas", type: "Venue", intro: "What's coming up at Atlas over the next few days.", eventIds: [10] }
];

function openStory(index) {
  const storyIndex = (Number(index) + followingStories.length) % followingStories.length;
  const story = followingStories[storyIndex];
  const storyEvents = story.eventIds.map(id => events.find(event => event.id === id)).filter(Boolean);
