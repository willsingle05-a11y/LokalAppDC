document.addEventListener("click", async event => {
  ...
  if (t.dataset.nativeShare) { mark(); const eventToShare = events.find(item => item.id === Number(t.dataset.nativeShare)); const message = shareMessageForEvent(eventToShare); try { if (navigator.share) await navigator.share({ title: eventToShare.title, text: message, url: location.href }); else await navigator.clipboard?.writeText(message); toast(navigator.share ? "Share sheet opened" : "Share message copied"); } catch { toast("Share canceled"); } }
  if (t.dataset.copyEventShare) { mark(); const eventToShare = events.find(item => item.id === Number(t.dataset.copyEventShare)); try { await navigator.clipboard?.writeText(shareMessageForEvent(eventToShare)); } catch {} toast("Share message copied"); }
