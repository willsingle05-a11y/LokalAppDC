...
function eventRow(event) {
  ...
  return `<button class="event-row feed-event" data-event="${event.id}" data-search-text="${`${event.title} ${event.venue} ${event.area} ${event.cat} ${event.tag}`.toLowerCase()}">
    <span class="event-art cat-${eventVisualCategory(event)}"><span class="art-label">${event.tag}</span></span>
...
function matchesFilter(event, filter, applyDiscoverFilters = true) {
  ...
  if (applyDiscoverFilters && state.filter.date === "Today" && !event.time.startsWith("Tonight")) return false;
  if (applyDiscoverFilters && state.filter.date === "This weekend" && event.time.startsWith("Tonight")) return false;
